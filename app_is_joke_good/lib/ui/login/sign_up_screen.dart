...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  bool _validatePassword({bool onlyUpdateErrors = false}) {
    ...
    if (pw.isPassword == false) {
      _passwordError = "Please ensure your password matches the criteria";
      setState(() {});
      return false;
    }
    setState(() {});
  ...
  void _validate({bool onlyUpdateErrors = false}) {
    ...
    if (pwValid == false || emailValid == false || onlyUpdateErrors) {
      if (onlyUpdateErrors == false) {
        showGeneralErrorAlert(
            title: "Invalid input", message: "Please check your input");
      }
      return;
    }
    _signUp();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: KeyboardDismissGestureDetector(
        child: SafeArea(
          child: Padding(
